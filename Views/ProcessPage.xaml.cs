  ...
  public partial class ProcessPage : Page
    {
        ...
        public async Task UpdateProcessList()
        {
            ...
            await Task.Run(() =>
            {
                AllProcessList = Process.GetProcesses().Where(p => p.MainWindowHandle != IntPtr.Zero)
                    .OrderBy(p => p.ProcessName);
                foreach (var process in AllProcessList) ProcessList.Add(process);
            });
        ...
        private void processListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                var process = processListBox.SelectedItem as Process;
                if (processListBox.SelectedItem == null)
                {
                    if (LastChoice == null || LastChoice.HasExited)
                    {
                ...
                var viewWindow = new ViewWindow(process);
                try
                {
                    viewWindow.Show();
                }
